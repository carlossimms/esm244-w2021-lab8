---
title: 'ESM 244 - Lab 8: Clustering (K-Means and Hierarchical)'
...
### Get and attach required packages
...
number_est <- NbClust(penguins[3:6], 
                      min.nc = 2, 
                      max.nc = 10, 
                      method = "kmeans")
...
We're still going to use 3 clusters and see how it does, though there may be a case here for 2 given that Adelie and Chinstrap penguins are pretty similar.
...
penguins_km <- kmeans(penguins_scale, 
                      3) # kmeans specifying 3 groups to start.
```
...
# On your own: do the same, plotting bill dimensions and mapping species and cluster number to the point shape and color aesthetics:
...
penguins_cl %>% 
  count(species, 
        cluster_no) %>% 
  pivot_wider(names_from = cluster_no, 
              values_from = n) %>% 
  rename('Cluster 1' = '1', 
         'Cluster 2' = '2', 
         'Cluster 3' = '3')
...
In this section, you'll be performing hierarchical cluster analysis (and making dendrograms) in R. You should have already watched the Week 8 lecture to understand agglomerative versus divisive clustering, as well as differences in linkages (complete, single, average). 
...
We see that it is a bit different when we change the linkage!
...
**Want to plot your dendrogram with ggplot instead?** 
...
```{r}
# Making the complete linkage dendrogram with `ggplot`:
...
# Making the single linkage dendrogram with `ggplot`:
...
# Then you can customize with usual ggplot tools. 
```
